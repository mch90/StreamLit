...
def plot_animation_2(df_2):
    ...
    #plot vertical line indicating sth (just test)
    #vert_line = alt.Chart(df_2).mark_rule(color='white').encode(x = "max(Category):T")
    #
    #ver_line = alt.Chart(df_2).mark_rule(color='white').encode(x = "mean(Category):T") ##calcluates mean value of given segment
    #text = alt.Chart(df_2).mark_text(color='white',align='center',baseline='top',dx=0).encode(text='utchoursminutesseconds(max(Category)):O')
    return lines2
            ...
            if start_btn:
                for i in range(1,N):
                    ...
                    #current_time_placeholder.text("Current time sample: ")
                        ...
                        st.write("Detected state: to be implemented")
                    ...
                    line_plot = line_plot.altair_chart(lines+lines3+vert_line+avg+text_y+text_curr_time)
                    ...
                    line_plot_2 = line_plot_2.altair_chart(lines2+lines4+vert_line+avg_2+text_x+text_curr_time)
